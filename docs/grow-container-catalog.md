...
- 关联：`docs/grow-container-necessity-review.md`（实现前的必要性验证流程与登记表）、
  `docs/grow-event-invocationlist.md`（机制复用与 LRU 纠正）、`docs/grow-event-design.md`、
  `docs/grow-architecture-rationale.md` §8（目录与归属）、`docs/architecture/ADR-0002-domain-reload-disabled.md`
...
**实现前门禁**：通过 G-1…G-4 后，还须通过 `docs/grow-container-necessity-review.md` 的 Necessity Gate
...
- 缺口：BCL/Unity 均无。
- 机制（**关键决策**）：`grow-event-invocationlist.md` §9 明确指出「LRU 需访问即 move-to-front，
...
- 缺口（见 `grow-container-necessity-review.md` §5）：`SortedDictionary` 按键排序而非插入序；
...
- 机制：`grow-event-invocationlist.md` §9 判定为「高机制吻合度」的
...
### 7.2 与 `grow-event-invocationlist.md` §9 的对齐与修订
...
1. **修订**：不再「等」，立即将**存储机制**回填为 `OrderedSet<T>`/`OrderedDictionary<K,V>`，
   ...
   或先私有。`grow-event-invocationlist.md` §9 的推迟结论由本文取代。
...
`InvocationList` 已存在，两者必要性验证均已通过（`grow-container-necessity-review.md` §5/§6），
...
| GC 断言在编辑器抖动 | 同 `grow-event-design.md` §9：预热 + 宽松阈值 + 可过滤 Category |
...
| `grow-event-invocationlist.md` §9 与本文的取向 | 已同步修订该文 §9：机制回填原语、`InvocationList` 改组合；实现落地后记入 `CHANGELOG.md` |
...
`grow-container-necessity-review.md` §5/§6）——`InvocationList` 的存储机制回填到前者、快照机制回填到
...
所有容器服从 §6 契约与 §2 门槛；LRU 采用 `grow-event-invocationlist.md` §9 的纠正路线
（字典 + 侵入式链表），不复用墓碑方案。
