...
- 关联：`docs/grow-container-catalog.md`（§2.1 原语优先、§5 清单、§10 路线图）、
  `docs/grow-event-invocationlist.md`（首个消费方）
...
| `UniTask PlayerLoopRunner` / `R3` 派发前取快照数组 | 快照派发 | 印证「枚举期安全」是**独立关切**，不属有序集合语义 | 已核验（见 `grow-event-design.md` §5） |
...
| 来源 | 形态 | 关系 | 核验 |
|---|---|---|---|
| `UniTask` `PlayerLoopRunner` | 遍历前取数组快照 | 印证策略；但快照内联于 runner，非可复用类型 | 已核验（`grow-event-design.md` §5） |
...
2. 已修正 `docs/grow-container-catalog.md` §5.7 与首发描述。
